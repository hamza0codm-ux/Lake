import { ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, AttachmentBuilder, MessageFlags, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, PermissionFlagsBits } from 'discord.js';
...
async function assertTicketPermission(interaction, client, actionLabel, options = {}, timeoutMs = 2500) {
  ...
  try {
    ...
    throw createError('Ticket permission check failed', ErrorTypes.UNKNOWN, `Failed to check permissions: ${error.message}`);
  }
  if (!context.ticketData) {
    throw createError('Not a ticket channel', ErrorTypes.VALIDATION, 'This action can only be used in a valid ticket channel.');
  }
  ...
  if (!allowed) {
    ...
    throw createError('Ticket permission denied', ErrorTypes.PERMISSION, `${permissionMessage}\n\nYou cannot ${actionLabel}.`);
  }
  return context;
}
...
const createTicketHandler = {
  ...
  async execute(interaction, client) {
    try {
      ...
      const reasonInput = new TextInputBuilder().setCustomId('reason').setLabel('Why are you creating this ticket?').setStyle(TextInputStyle.Paragraph).setPlaceholder('Describe your issue...').setRequired(true).setMaxLength(1000);
...
const createTicketModalHandler = {
  ...
  async execute(interaction, client) {
    try {
      ...
      const { channel } = await createTicket(interaction.guild, interaction.member, categoryId, reason);
      await interaction.editReply({ embeds: [successEmbed('Ticket Created', `Your ticket has been created in ${channel}!`)] });
    } catch (error) {
...
const closeTicketHandler = {
  ...
  async execute(interaction, client) {
    try {
      ...
      const reasonInput = new TextInputBuilder().setCustomId('reason').setLabel('Reason for closing (optional)').setStyle(TextInputStyle.Paragraph).setPlaceholder('Add an optional reason for closing this ticket...').setRequired(false).setMaxLength(1000);
      const actionRow = new ActionRowBuilder().addComponents(reasonInput);
      modal.addComponents(actionRow);
      await interaction.showModal(modal);
    } catch (error) {
  ...
  async execute(interaction, client, args) {
    try {
      ...
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
        await replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'You must have **Manage Server** permissions.' });
        return;
      }
  ...
  async execute(interaction, client) {
    try {
      ...
      if (hasPingEmoji) {
        ...
        await interaction.editReply({ embeds: [createEmbed({ title: '📌 Ticket Unpinned', description: 'This ticket has been unpinned.', color: 0x95A5A6 })], flags: MessageFlags.Ephemeral });
      } else {
        const pinnedName = `📌 ${channel.name}`;
        await channel.edit({ name: pinnedName, position: 0 });
        await interaction.editReply({ embeds: [createEmbed({ title: '📌 Ticket Pinned', description: 'This ticket has been pinned.', color: 0x3498db })], flags: MessageFlags.Ephemeral });
      }
    } catch (error) {
      logger.error('Error pinning ticket:', error);
    }
  }
};
...
const reopenTicketHandler = {
  ...
  async execute(interaction, client) {
    try {
      ...
      await interaction.editReply({ embeds: [successEmbed('Ticket Reopened', 'This ticket has been reopened.')] });
    } catch (error) {
...
const priorityTicketButtonHandler = {
  name: 'ticket_priority_btn',
  async execute(interaction, client) {
    try {
      if (!(await ensureGuildContext(interaction))) return;
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
        await replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'Only operators with **Manage Server** permissions can use this.' });
        return;
      }
      await assertTicketPermission(interaction, client, 'change ticket priority', {}, 2000);
      ...
      const actionRow = new ActionRowBuilder().addComponents(priorityMenu);
      await interaction.reply({ content: 'Select the priority tier for this ticket:', components: [actionRow], flags: MessageFlags.Ephemeral });
    } catch (error) {
...
const priorityTicketSelectHandler = {
  name: 'ticket_priority_select',
  async execute(interaction, client) {
    try {
      if (!(await ensureGuildContext(interaction))) return;
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
        await replyUserError(interaction, { type: ErrorTypes.PERMISSION, message: 'Only operators with **Manage Server** permissions can modify priority.' });
        return;
      }
      await assertTicketPermission(interaction, client, 'change ticket priority', {}, 2000);
      const deferSuccess = await InteractionHelper.safeDefer(interaction, { flags: MessageFlags.Ephemeral });
      ...
      await updateTicketPriority(interaction.channel, selectedPriority, interaction.user);
      await interaction.editReply({ content: `Ticket priority successfully updated to **${selectedPriority.toUpperCase()}**!`, components: [] });
    } catch (error) {
      logger.error('Error processing priority option:', error);
    }
  }
};
...
export { 
  createTicketModalHandler, closeTicketModalHandler, closeTicketHandler, claimTicketHandler, 
  priorityTicketHandler, pinTicketHandler, unclaimTicketHandler, reopenTicketHandler, 
  deleteTicketHandler, priorityTicketButtonHandler, priorityTicketSelectHandler 
};
